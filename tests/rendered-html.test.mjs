...
test("server-renders the ExamSaathi application shell", async () => {
  ...
  assert.match(html, /<title>ExamSaathi — Clear Daily Practice for SSC CGL &amp; CHSL<\/title>/i);
...
test("ships focused CGL and CHSL content with cloud learning support", async () => {
  ...
  assert.match(app, /Find a topic and practise/);
  assert.match(app, /Search questions, topics or Hindi text/);
  assert.match(app, /Know what to study/);
  assert.match(app, /Three simple steps\. No confusion/);
  ...
  assert.match(app, /Login is handled securely by Supabase/);
  assert.match(app, /signInWithOAuth\(\{ provider: "google"/);
  assert.match(app, /signInWithOtp\(\{ phone:/);
  assert.match(app, /verifyOtp\(\{ phone:/);
  assert.match(app, /examsaathi-theme/);
  ...
  assert.doesNotMatch(app, /QUESTION_BANK_SIZE|\b1,?600\b/);
  assert.match(styles, /\.app-shell \{ height: 100dvh; min-height: 100dvh; overflow: hidden; \}/);
  assert.match(styles, /\.app-main \{ height: 100dvh; min-height: 0;/);
  assert.doesNotMatch(styles, /\.app-shell > :not\(\.immersive-backdrop\)/);
